...
export class CommandPaper {
    ...
    /**
     * Runs a  command on a player with args
    ...
    run(cmd, player, args) {
        ...
        if (!cmd)
            return player.error(lang.cmd.unknown(this.prefix, player.isAdmin), '');
        if (!cmd.rM.cM)
            return player.error(lang.cmd.useForm, '');
        if (player.isAdmin ? false : cmd.admin || (cmd.tags?.length ? !player.hasTags(cmd.tags) : false))
            return player.error(lang.cmd.noPerms, '');
        if (!cmd.sT[0].length && args.length)
            return player.error(lang.cmd.noArgs, '');
        if (cmd.sT[1] && !args.length)
            if (cmd.sT[0].some(a => cmd.aR[a].tY === 'plr' && (cmd.aR[a].tV[0]?.self ?? true)))
                ...
                return player.error(lang.cmd.missingArgs(this.prefix, cmd.name, args, this.findExample(cmd.aR[cmd.sT[0][0]], player)), '');
        ...
        while (fetchCLs) {
            ...
            if (!nAR?.length && nextArgs.length)
                return player.error(lang.cmd.maxArgs(nextArgs[0]), '');
            const type = this.findType(cmd, player, nAR, nextArgs), arg = type?.aRN ? cmd.aR[type.aRN] : null;
            if (!arg)
                return player.error(lang.cmd.notAArg(this.prefix, cmd.name, args.slice(0, args.length - nextArgs.length), nextArgs[0], nextArgs.slice(1).join(' '), this.findExample(cmd.aR[nAR[0]], player)), '');
            ...
            if (['dyn', 'sta'].includes(arg.tY) && !type.tV?.length && arg.tV[1])
                return player.error(lang.cmd.needVal(this.prefix, arg.tV[0]), '');
            ...
            if (arg.tG?.length && !arg.tG.some(t => player.hasTag(t)))
                return player.error(lang.cmd.noArgPerm, '');
...
class command {
     ...
     * @param {boolean} needNextArg If the next argument(s) from before are needed
     * @example .numberType('rate ', (plr, num, args) => {
     ...
     * @param {boolean} needNextArg If the next argument(s) from before are needed
     * @example .timeType('rate ', (plr, ms, args) => {
     * <tab> console.warn(num); //Output: number
     * }, [], [0, 10]);
     * @returns {command}
     */
