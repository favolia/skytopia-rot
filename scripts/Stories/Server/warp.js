import { world } from "@minecraft/server";
import { sleep } from "../../Papers/Paragraphs/ExtrasParagraphs.js";
import Commands from "../../Papers/CommandPaper/CommandPaper.js";
import Server from "../../Papers/ServerPaper.js";
import quick from "../../quick.js";
if (!Server.db.has('warps'))
    Server.db.write('warps', {});
const cmd = Commands.create({
    name: 'warp',
    description: 'Gunakan perintah ini untuk melakukan teleportasi ke berbagai lokasi di peta.',
    aliases: ['w', 'wrp'],
    category: 'Server',
    developers: ['Mo9ses']
});
cmd.startingArgs(['set', 'list', 'remove', 'warp']);
cmd.unknownType('tag', null, 1);
cmd.locationType('loc', null, 'tag', false, false);
cmd.playerType('plr');
cmd.staticType('set', 'set', async (plr, val, args) => {
    if (val.length > 12)
        return plr.error('Nama warp harus terdiri dari maksimal §4§l12§r§c karakter atau kurang.');
    const warps = Server.db.read('warps'), amount = Object.keys(warps).length;
    warps[val] = [args[1], [parseInt(`${args[0].x}`), parseInt(`${args[0].y}`), parseInt(`${args[0].z}`), plr.dimension.id]];
    Server.db.write('warps', warps);
    plr.send(`Kamu telah mengatur warp dengan nama §c${val}§e pada koordinat: §a${parseInt(`${args[0].x}`)}§e, §a${parseInt(`${args[0].y}`)}§e, §a${parseInt(`${args[0].z}`)}§e, §ddi dimensi ${plr.dimension.id.replace('minecraft:', '')}§e.`);
    await sleep(40);
    if (amount > 30)
        plr.tip(`Server ini memiliki lebih dari §c§l30§r§e (§4§l${amount}§r§e) warp! Memiliki lebih dari §c30§e warp dapat menyebabkan server menjadi lambat. §o§7Sebenarnya saya belum mengujinya, namun seharusnya dapat menangani sekitar 100 warp.`);
}, 'loc', true);
cmd.staticType('list', 'list', plr => {
    const warps = Object.entries(Server.db.read('warps')).filter(w => plr.isAdmin || !w[1][0] ? true : plr.hasTag(w[1][0]));
    if (!warps.length)
        return plr.error('Server ini tidak memiliki warp yang tersedia untukmu.');
    plr.send(`Server ini memiliki total ${warps.length} warp. Berikut daftar warp yang tersedia di server ini:\n${warps.map(w => `- ${w[0]}: Koordinat (${w[1][1][0]}, ${w[1][1][1]}, ${w[1][1][2]}) di dimensi ${w[1][1][3].replace('minecraft:', '')}. Tag: ${w[1][0] || 'none'}`).join('\n')}`);
}, [], false);
cmd.staticType('remove', 'remove', (plr, val) => {
    const warps = Server.db.read('warps');
    if (!warps.hasOwnProperty(val))
        return plr.error('Server ini tidak memiliki warp dengan nama tersebut.');
    plr.send(`Berhasil menghapus warp dengan nama §c${val}§e pada koordinat: §a${warps[val][1][0]}§e, §a${warps[val][1][1]}§e, §a${warps[val][1][2]}§e, §ddi dimensi ${warps[val][1][3].replace('minecraft:', '')}§e..`);
    delete warps[val];
    Server.db.write('warps', warps);
});
cmd.unknownType('warp', (plr, val, args) => {
    const warp = Object.entries(Server.db.read('warps')).find(w => w[0] === val && (plr.isAdmin || !w[1][0] ? true : plr.hasTag(w[1][0])));
    if (!warp)
        return plr.error('Server ini tidak memiliki warp dengan nama tersebut. Pastikan warp tersebut ada dan kamu memiliki izin untuk melakukan teleportasi ke sana.');
    const target = args.length ? args[0] : plr;
    if (target.name !== plr.name && !plr.isAdmin)
        return plr.error('Kamu tidak memiliki izin untuk melakukan teleportasi pada pemain lain.');
    target.addTag(quick.epics['Automod'].protections.teleport.skip);
    target.teleport({ x: warp[1][1][0], y: warp[1][1][1], z: warp[1][1][2] }, { dimension: world.getDimension(warp[1][1][3]), rotation: { x: target.getRotation().x, y: target.getRotation().y } });
    target.send(`Kamu telah melakukan teleportasi ke §a§l${val}§r§e.`);
}, 1, true, 'plr');
cmd.config(['set', 'remove'], {
    admin: true
});
